	...
	foreach($movies as $movie)
		...
		$title1[] = $movie->title ;                   // Movie title from api
                ...
                for ($x = 0; $x <= $torrents_counts11; $x++) {
	        	    $torrent1111[$x] = ''.$movie->torrents[$x]->hash;
	        	    $quality1111[$x] = $movie->torrents[$x]->quality;
	        	    $size1111[$x]    = $movie->torrents[$x]->size;
                    $size1111[$x]    = $movie->torrents[$x]->size;

	        	    $complete_torrent_info_test[$x] ='

					
					<div class="torrent_all">
						<div class="q_s_text">
							Quality '.$quality1111[$x].' , Size '.$size1111[$x].'
						</div>
						<div>	
							<a href="magnet_streaming.php?magnet_uri=magnet:?xt=urn:btih:'.$torrent1111[$x].'" Class="stream_q"><i class="fa fa-play-circle" style="font-size:15px"></i>&nbsp;Stream</a>
							<a href="https://yts.mx/torrent/download/'.$torrent1111[$x].'" Class="torrent_q"><i class="fa fa-download" style="font-size:15px"></i>&nbsp;Torrent</a>
							<a href="magnet:?xt=urn:btih:'.$torrent1111[$x].'" Class="magnet_q"><i class="fa fa-magnet" style="font-size:15px"></i>&nbsp;Magnet</a>
						</div>
					</div>					
